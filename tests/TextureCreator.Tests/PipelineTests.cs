...
public sealed class PipelineTests
{
    ...
    [Fact] public void PbrMapsAreConsistentDimensions() { var src = Pattern(32); var set = new PbrGenerator().Generate(src, MaterialKind.BareMetal, .3f, .8f, 1); Assert.Equal(7, set.Maps.Count); Assert.All(set.Maps.Values, x => { Assert.Equal(32, x.Width); Assert.Equal(32, x.Height); }); Assert.True(set[MapKind.Metalness].Pixels[0] > 200); }
    ...
    [Fact] public void ExportWritesPngSet() { var set = new PbrGenerator().Generate(Pattern(8), MaterialKind.Wood, .6f, 0, 1); var dir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString()); var files = TextureExporter.Export(set, dir, "Cube"); Assert.Equal(7, files.Count); Assert.All(files, f => Assert.True(new FileInfo(f).Length > 20)); }
